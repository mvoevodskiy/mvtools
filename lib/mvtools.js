...
MVTools.__merge = function (recursive, ...objects) {
  ...
  for (const nObject of objects) {
    ...
    if (Array.isArray(nObject)) {
      ...
      for (let key of Array.prototype.concat(Object.keys(nObject), Object.getOwnPropertySymbols(nObject))) {
...
MVTools.escapeRegExp = function (string) {
 ...
 */
MVTools.listToColumns = function (list, columns = 2) {
  const rows = []
  let row = []
...
MVTools.call = function (method, ...params) {
  ...
  if (typeof method === 'object') {
    if ('params' in method) params.push(method.params)
    method = method.method || method.name
  }
  // console.log('MT CALL. METHOD:', method, 'TYPE:', typeof method)
  if (typeof method === 'string') {
    method = this.extract(method)
  }
  // console.log('MT CALL. EXTRACTED (?) METHOD:', method, 'TYPE:', typeof method)
